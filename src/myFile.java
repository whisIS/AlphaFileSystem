...
public class myFile implements File {
        ...
        if(map == null){
            ...
            for(Map.Entry<Id,Id> entry : map.entrySet()){
                if((entry.getKey()).equals(new StringId("FILE_EMPTY"))){
                    //处理文件空洞,newEmptyBlock不应该用在bm里因为文件空洞的时候是不存在block里的
                    blockData = new byte[(int)blockSize];
                    break;
                } else {
                    BlockManager blockManager = config.myBlockManagerMap.get(entry.getKey());
    ...
    private Map<Id,Id> writeDuplication(byte[] b){
        ...
        if(isEmpty1){
            ...
            StringId stringId1 = new StringId("bm-1");
            Block block1 = writeBlock(stringId1, b);
            StringId stringId2 = new StringId("bm-2");
            Block block2 = writeBlock(stringId2, b);
            StringId stringId3 = new StringId("bm-3");
            Block block3 = writeBlock(stringId3, b);

            ret.put(stringId1, block1.getIndexId());
            ret.put(stringId2, block2.getIndexId());
            ret.put(stringId3, block3.getIndexId());
            return ret;
        }
    }
    private Block writeBlock(StringId id,byte[] b){
        BlockManager blockManager = config.myBlockManagerMap.get(id);
            ...
            for(int k = 0; k < LogicBlockList.size(); k++){
                ...
                if(LogicBlockList.get(k).equals(config.fileEmptyMap)){
                    ...
                    for(Map.Entry<Id,Id> entry : LogicBlockList.get(k).entrySet()){
                        sb.append("[\"").append(((StringId) entry.getKey()).getId()).append("\",").append(((StringId) entry.getValue()).getId()).append("] ");
                    }
                }
